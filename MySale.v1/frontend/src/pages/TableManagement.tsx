  ...
  if (orderMode && selectedTable) {
    return (
      <div className="fixed inset-0 z-50 flex flex-col bg-gray-50">
        <div className="flex items-center justify-between px-2 lg:px-4 py-2 bg-white border-b border-gray-200 shadow-sm">
          <div className="flex items-center gap-2 lg:gap-4 min-w-0">
            <Button
              onClick={handleExitOrderMode}
              className="bg-red-600 hover:bg-red-500 text-white h-10 px-3 lg:px-4 shrink-0"
            >
        ...
        {!currentTicket ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-white p-6 lg:p-8 rounded-xl w-full max-w-sm mx-3 space-y-4 shadow-lg border border-gray-200">
          ...
          {isSuperuser && (
            <>
              <Button
                ...
                className={`hidden lg:inline-flex ${editMode ? "bg-amber-500 hover:bg-amber-600 text-white" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}
              >
        ...
        <div className="flex overflow-x-auto border-b border-gray-200 bg-white px-2">
          {zones.map(zone => (
            <button
              ...
              className={`shrink-0 whitespace-nowrap px-4 lg:px-5 py-3 text-sm font-medium transition-colors relative ${
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2 lg:hidden">
        ...
        {currentZone && currentZone.tables.length > 0 ? (
          currentZone.tables.map(table => {
            ...
            const elapsed = isTableFree(table.status)
              ? null
              : getElapsedTime(table.ticket_opened_at) || table.ticket_time || null;

            return (
              <button
                key={table.id}
                onClick={() => handleTableClick(table)}
                className="w-full flex items-center gap-3 bg-white rounded-xl border border-gray-200 px-4 py-3 text-left active:scale-[0.99] transition-transform"
              >
                <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: overlay.color }} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-gray-900 truncate">{table.name}</span>
                ...
                <span
                  className="text-white text-[11px] font-medium px-2 py-1 rounded-full shrink-0"
          ...
          <div className="py-16 text-center">
            {zones.length === 0 ? (
              <p className="text-gray-400 font-medium">No hay zonas creadas</p>
            ) : (
              <p className="text-gray-400 font-medium">No hay mesas en esta zona</p>
            )}
          </div>
        )}
      </div>

      <div className="hidden lg:block flex-1 overflow-auto relative">
        {/* Edit mode toolbar */}
